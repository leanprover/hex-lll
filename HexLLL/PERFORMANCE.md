...
- **Lean certified:** an `fplll` candidate checked by the verified Lean
  checker (`certCheck`); the certified external-reduction method.
...
The table and figures measure clean source revision
`1c7e2b9fd7326ab197b4609a9f104da4f442346e` with Lean 4.32.0-rc1 on
`carica`; they are retained as the latest complete five-system comparison,
not relabelled as Lean 4.32.2 timings. The correctness and benchmark
verification checks have been rerun on Lean 4.32.2.
...
remains the trustworthy in-tree reducer when no external reducer is loaded.
...
- **fpLLL vs Lean certified**: same asymptotic slope and small constant gap; the
